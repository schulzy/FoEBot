...
namespace Schulzy.FoEBot.BL.Communication
{
    public class HttpRequestManager : IHttpRequestManager
    {
        ...
        {
            var cookie = Cookies.GetCookies(siteUri)[name];
            return cookie?.Value;
        }
        ...
        public string GetResponseContent(HttpWebResponse response)
        {
            ...
            try
            {
                ...
                if (dataStream == null)
                    throw new ArgumentNullException(nameof(dataStream), "The response data is null");
        ...
        {
            var request = GeneratePostRequest(uri, content, login, password, allowAutoRedirect);
            return GetResponse(request);
        }

        public HttpWebResponse SendGetRequest(string uri, string login, string password, bool allowAutoRedirect)
        {
            var request = GenerateGetRequest(uri, login, password, allowAutoRedirect);
            return GetResponse(request);
        }
        ...
        public HttpWebResponse SendRequest(string uri, string content, string method, string login, string password,
            bool allowAutoRedirect)
        {
            var request = GenerateRequest(uri, content, method, login, password, allowAutoRedirect);
            return GetResponse(request);
        }
        ...
        internal HttpWebRequest GenerateRequest(string uri, string content, string method, string login,
            string password, bool allowAutoRedirect)
        {
            if (uri == null)
